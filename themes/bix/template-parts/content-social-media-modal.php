<!--Start of Modal -->
<div class="modal">
    <input class="modal__trigger" id="modal__trigger" type="checkbox" />
    <label for="modal__trigger" class="fa fa-share-square-o" aria-hidden="true"></label>
    <div class="modal__overlay" role="dialog" aria-labelledby="modal__title" aria-describedby="modal_desc">
        <div class="modal__wrap">
            <label class="close-out" for="modal__trigger">&#10006;</label>
            <h1> Share This </h1>
            <h5 class="modal__wrap-h5"> Share this on your favourite social media page</h5>
        <ul class="icons-list">
	           <li><a class="icon-size" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	           <li><a class="icon-size" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	           <li><a class="icon-size" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
       </ul>
       <button class="modal-button" href="#"> Done </button>
   </div>
 </div>
</div>

<!--End of Modal -->
